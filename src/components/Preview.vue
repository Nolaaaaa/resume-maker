<template>
    <div id="preview">
        <section class="profile">
            ...
            <div class="contacts" v-if="filter(resume.contacts).length > 0" v-for="items in filter(resume.contacts)" :key="items.id">
...
export default {
    props: ['resume'],
    methods: {
        filter(array) {  //过滤空对象。有项目就显示
            return array.filter(item => !this.isEmpty(item))
        },
        isEmpty(object){     //判断是否为空，true为空，false为中间有值
            let empty = true
...
<style lang="scss">
    #preview{
        ...
        * {padding: 5px 0 ;}
        ...
        // border:3px solid rgb(141, 182, 245);
    }
</style>